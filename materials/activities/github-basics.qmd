...
Examine the updated local repository files; notice that there is now a `class-activity` folder containing a copy of this activity.
...
2.  Then choose someone else to try -- use the main screen if possible. Since the first person modified the remote repository, the next person to push changes will no longer be up to date. Git will detect this and the push won't go through.
...
4.  Have everyone at your table pull changes.
:::
...
3.  Each group member has an up-to-date local copy of the repository.
